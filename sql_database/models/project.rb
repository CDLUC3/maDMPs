class Project < ActiveRecord::Base
  ...
  def self.create_by_hash!(hash)
    ...
    project.project_contributors = Array(hash.fetch(:contributors, [])).map do |contributor|
      ProjectContributor.new(source_id: hash[:source_id],
                             role: contributor.fetch(:role, 'Contributor'),
                             contributor_id: Contributor.find_or_create_by_hash(contributor).id)
    end
    project.save!
    project
  end
